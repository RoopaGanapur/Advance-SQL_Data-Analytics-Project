...
/***   1 .Find top 3 outlets by cuisine type without using top and limit function****/
...
/***   2 .Find the daily  newly customer count from launch date  laucnched on jan 1 st****/
with cte as
(select Customer_code,min(placed_at) as firstorderdate
from orders 
group by customer_code)
select cast(firstorderdate as date),count(*) as no_of_new_customers
from cte
group by cast(firstorderdate as date) ;

/***   3.count all the users who were acquired in jan 2025 and only placed one order in jan and didnot place any other order****/
...
max(placed_at) as latest_order_date from 
orders
group by customer_code)
select cte.*,orders.Promo_code_Name from cte join orders 
...
/***   5.GROWTH TEAM IS PLANNING TO CREATE A TRIGGER THAT WILL TARGET THEIR CUSTOMERS AFTER THEIR EVRY THIRD ORDER WITH A PERSONALISED COMMUNICATION AND THEY ASKED TO 
TO CREATE A QUERY FOR THIS****/
WITH CTE AS
(SELECT *,ROW_NUMBER() OVER(PARTITION BY CUSTOMER_CODE ORDER BY PLACED_AT) AS ROW_NUMBER
FROM  ORDERS)
SELECT * FROM CTE WHERE ROW_NUMBER%3=0 and  cast(placed_at as date)=cast(getdate() as date)
...
/*** 7.what percent of customers were organically acquired in jan 2025 without promocode****/
with cte as(
select *,row_number() over(partition by customer_code order by placed_at) as rn
from orders where month(placed_at)=1)
